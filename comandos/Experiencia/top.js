...
function mostrarRankExp(interaction) {
    Usuario.find().sort({ expTotal: -1 }).exec(async (err, docs) => {
        var j = 0;
        const top3 = [
            {
                user: await test(0),
...
function mostrarRankExpTemporada(interaction, temporada) {
    Usuario.find().sort({ [`expTemporadas.${temporada}`]: -1 }).exec(async (err, docs) => {
        var j = 0;
        const top3 = [
            {
                user: await test(0),
